...
FILENAME = ARGV[0]
printf("question_url, question_created, answer_url, answer_creator, answer_created\n")
CSV.foreach(FILENAME, :headers => true) do |row|
  ...
  answers_array["results"].each do |answer|
    ...
    answer_creator = answer["creator"]["username"]
    next if answer_creator == creator
    question_url = sprintf("https://support.mozilla.org/questions/%s", id)
    answer_url = sprintf("%s#answer-%s", question_url, answer["id"])
    ...
    puts(question_url + ',' + row["created"] + ',' + answer_url + ',' + 
      answer_creator + ',' + answer_created.to_s)
  end
